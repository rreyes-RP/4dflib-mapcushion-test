...
public class DutchessCountyTenant {
    ...
    private static void insertSomeData() throws InterruptedException {
        ...
        List<Visit> visit01 = cis.getVisitByUserType("Faculty");
        for(Visit visit: visit01) {
            if(visit.currentUserId >= 0) {
                visit.currentUser = iu.getUsersById(visit.currentUserId);
            }

            String users = (visit.currentUser != null) ? visit.currentUser.fName : "no users";

            System.out.println("User: " + users + " Check In Status: " + visit.checkStatus);
        }

        Thread.sleep(3000);

        for(Visit visit: visit01) {
            if(visit.checkStatus == CheckStatus.CheckedIn) {
                System.out.println("User: " + visit.currentUser.fName + " is already Checked In");
            }
            else {
        ...
        List<FdfEntity<Visit>> visit01withHistory = cis.getVisitByUserTypeWithHistory("Faculty");
        for(FdfEntity<Visit> visitWithHistory: visit01withHistory) {
            System.out.println("User : " + visitWithHistory.current.name + "\nCurrent Check In Status:  " + visitWithHistory.current.checkStatus + "\nUser Type: " + visitWithHistory.current.typeUser);

            System.out.println("------- History ---------");
            for (Visit visitHistory : visitWithHistory.history) {
                System.out.println("Start Check Status: " + visitHistory.arsd + " Check Status Change Time: " + visitHistory.ared + " User: " + visitHistory.name + " Check Status: " +
                        visitHistory.checkStatus);
            }

            System.out.println("________________");
        }

        List<FdfEntity<Visit>> visitorwithHistory = cis.getVisitByUserTypeWithHistory("Visitor");
        for(FdfEntity<Visit> visitWithHistory: visitorwithHistory) {
            System.out.println("User : " + visitWithHistory.current.name + "\nCurrent Check In Status:  " + visitWithHistory.current.checkStatus + "\nUser Type: " + visitWithHistory.current.typeUser);
        ...
        // Changing Will's name to Ron Burgundy
        System.out.println("User: " + will2.fName + " " + will2.lName + " is changing his name");
        will2.fName = "Ron";
        will2.lName = "Burgundy";
        // Wait a few seconds
        Thread.sleep(3000);
        skiu.saveUsers(will2);

        // See if Will Ferrell changed to Ron Burgundy
        will2 = skiu.getUsersByPerIdNum("RS7310-71047");
        System.out.println("User: " + will2.fName + " " + will2.lName + " changed his name.");

        // assign Ron to system role
        ...
        // Changing Brian's name to Paul Rudd
        System.out.println("User: " + brian.fName + " " + brian.lName + " is changing his name");
        brian.fName = "Paul";
        brian.lName = "Rudd";
        // Wait a few seconds
        Thread.sleep(3000);
        ...
        // See if Brian Fantana changed to Paul Rudd
        brian = skiu.getUsersByPerIdNum("RS4525-69102");
        System.out.println("User: " + brian.fName + " " + brian.lName + " changed his name.");

        // assign Paul to system role
        ...
        List<SK_Visit> visit02 = skcis.getVisitByUserType("Visitor");
        for(SK_Visit visit: visit02) {
            ...
            System.out.println("Users: " + users + " " + visit.checkStatus);
        }
        ...
        List<SK_Visit> visit02new = skcis.getVisitByUserType("Visitor");
        visit02new.get(0).checkStatus = SK_CheckStatus.CheckedOutCompleted;
        skcis.saveVisit(visit02new.get(0));

        System.out.println("-------------------------------");

        List<FdfEntity<SK_Visit>> visit02withHistory = skcis.getVisitByUserTypeWithHistory("Visitor");
        ...
        List<FdfEntity<SK_Visit>> facultywithHistory = skcis.getVisitByUserTypeWithHistory("Faculty");
        for(FdfEntity<SK_Visit> visitWithHistory: facultywithHistory) {
            System.out.println("User : " + visitWithHistory.current.name + "\nCurrent Check In Status:  " + visitWithHistory.current.checkStatus);
        ...
        System.out.println("################# 4a) Query Tenant Name #################");
        ...
        */
        ...
        System.out.print("\n");
        System.out.println("################# 4b) Query Users from Arlington High School #################");
        System.out.println("--------------------------------------------------------------------------------------");
        ...
        */
        ...
        System.out.print("\n");
        System.out.println("################# 4b) Query Users from Spackenkill High School #################");
        System.out.println("--------------------------------------------------------------------------------------");
        System.out.println("User" + "           |");
        ...
        System.out.print("\n");
        System.out.println("################# 4c) Query 2 Users and the following attributes #################\n" +
        ...
        List<Users> arUserChangePB = iu.customUserQuery("RS8532-82547").history;
        List<Users> arUserChangeAM = iu.customUserQuery("RS3492-71946").history;
        List<SK_Users> skUserChangeBT = skiu.customUserQuery("RS6544-09587").history;
        List<SK_Users> skUserChangeWF = skiu.customUserQuery("RS7310-71047").history;
        List<SK_Users> skUserChangeBF = skiu.customUserQuery("RS4525-69102").history;

        Users arUserCurrentPB = iu.customUserQuery("RS8532-82547").current;
        Users arUserCurrentAM = iu.customUserQuery("RS3492-71946").current;
        SK_Users skUserCurrentBT = skiu.customUserQuery("RS6544-09587").current;
        SK_Users skUserCurrentWF = skiu.customUserQuery("RS7310-71047").current;
        SK_Users skUserCurrentBF = skiu.customUserQuery("RS4525-69102").current;

        System.out.println("\n################# 4d) Query Original User Record vs Updated User Record #################");
        System.out.println("\n--------------- Original User Record --------------- ");
        System.out.println("First Name     |     Last Name     |     Start Time     |     End Time");
        System.out.println("--------------------------------------------------------------------------------------");
        System.out.println(arUserChangePB.get(0).fName + "            |      " + arUserChangePB.get(0).lName + "     |     " + arUserChangePB.get(0).arsd + "                |     " + arUserChangePB.get(0).ared);
        System.out.println("--------------------------------------------------------------------------------------");
        System.out.println(arUserChangeAM.get(0).fName + "         |      " + arUserChangeAM.get(0).lName + "         |     " + arUserChangeAM.get(0).arsd + "     |     " + arUserChangeAM.get(0).ared);
        System.out.println("--------------------------------------------------------------------------------------");
        System.out.println(skUserChangeBT.get(0).fName + "          |      " + skUserChangeBT.get(0).lName + "      |     " + skUserChangeBT.get(0).arsd + "     |     " + skUserChangeBT.get(0).ared);
        System.out.println("--------------------------------------------------------------------------------------");
        System.out.println(skUserChangeWF.get(0).fName + "          |      " + skUserChangeWF.get(0).lName + "      |     " + skUserChangeWF.get(0).arsd + "     |     " + skUserChangeWF.get(0).ared);
        System.out.println("--------------------------------------------------------------------------------------");
        System.out.println(skUserChangeBF.get(0).fName + "          |      " + skUserChangeBF.get(0).lName + "      |     " + skUserChangeBF.get(0).arsd + "     |     " + skUserChangeBF.get(0).ared);

        System.out.println("\n--------------- Updated User Record --------------- ");
        System.out.println("First Name     |     Last Name     |     Start Time     |     End Time");
        System.out.println("--------------------------------------------------------------------------------------");
        System.out.println(arUserCurrentPB.fName + "            |      " + arUserCurrentPB.lName + "     |     " + arUserCurrentPB.arsd + "                |     " + arUserCurrentPB.ared);
        System.out.println("--------------------------------------------------------------------------------------");
        System.out.println(arUserCurrentAM.fName + "         |      " + arUserCurrentAM.lName + "         |     " + arUserCurrentAM.arsd + "     |     " + arUserCurrentAM.ared);
        System.out.println("--------------------------------------------------------------------------------------");
        System.out.println(skUserCurrentBT.fName + "          |      " + skUserCurrentBT.lName + "      |     " + skUserCurrentBT.arsd + "     |     " + skUserCurrentBT.ared);
        System.out.println("--------------------------------------------------------------------------------------");
        System.out.println(skUserCurrentWF.fName + "          |      " + skUserCurrentWF.lName + "      |     " + skUserCurrentWF.arsd + "     |     " + skUserCurrentWF.ared);
        System.out.println("--------------------------------------------------------------------------------------");
        System.out.println(skUserCurrentBF.fName + "          |      " + skUserCurrentBF.lName + "      |     " + skUserCurrentBF.arsd + "     |     " + skUserCurrentBF.ared);
        ...
        System.out.println("\n############### 4e) Query Current Check In Status from Arlington High School ###############");
        System.out.println("--------------------------------------------------------------------------------------");
        ...
        System.out.println("\n############### 4e) Query Current Check In Status from Spackenkill High School ###############");
        System.out.println("--------------------------------------------------------------------------------------");
        System.out.println("Name           |     Destination     |     Time                      |     Check In Status");
        System.out.println("--------------------------------------------------------------------------------------");
        ...
        System.out.println("\n############### 4f) Query User That Cycled Through The Full Check In ###############");
        List<FdfEntity<SK_Visit>> visit02RecapHistory = skcis.getVisitByUserTypeWithHistory("Visitor");
        for(FdfEntity<SK_Visit> visitWithHistory: visit02RecapHistory) {
            System.out.println("\n------- Spackenkill Full Visit History ---------");
            System.out.println("User          |     Check Status     |          Start Time            |     End Time");
            System.out.println("-----------------------------------------------------------------------------------------");
            for (SK_Visit visitHistory : visitWithHistory.history) {
                System.out.println(visitHistory.name + "   |     " + visitHistory.checkStatus + "      |     "
                        + visitHistory.arsd + "     |     " + visitHistory.ared);
                System.out.println("--------------------------------------------------------------------------------------");
            }
            System.out.println(visitWithHistory.current.name + "   |  " + visitWithHistory.current.checkStatus + " |    "
                    + visitWithHistory.current.arsd + "     |     " + visitWithHistory.current.ared);
            System.out.println("________________");
        }
        ...
        System.out.println("\n############### 4g) Query Beacon information from Arlington High School ###############");
        System.out.println("--------------------------------------------------------------------------------------");
        ...
        System.out.println("\n############### 4g) Query Beacon information from Spackenkill High School ###############");
        System.out.println("--------------------------------------------------------------------------------------");
        System.out.println("Location                    |     Floor     |     Beacon RSSI");
        System.out.println("--------------------------------------------------------------------------------------");
